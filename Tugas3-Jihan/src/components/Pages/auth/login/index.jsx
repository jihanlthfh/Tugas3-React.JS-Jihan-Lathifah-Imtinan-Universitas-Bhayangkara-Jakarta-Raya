import { Link } from "react-router-dom";

export default function Login(){
    return(
        <>
        <div className="modal modal-sheet position-static d-block p-4 py-md-5">
            <div className="modal-dialog">
                <div className="modal-content rounded-4 shadow">

                    <div className="modal-header p-5 pb-4 border-bottom-0 d-flex justify-content-center">
                        <h1 className="fw-bold mb-0 fs-2">Login</h1>
                    </div>

                    <div className="modal-body p-5 pt-0">
                        <form onSubmit={(e) => e.preventDefault()}>

                            <div className="form-floating mb-3">
                                <input type="email" className="form-control rounded-3" id="floatingInput" placeholder="name@example.com" />
                                <label htmlFor="floatingInput">Email address</label>
                            </div>

                            <div className="form-floating mb-3">
                                <input type="password" className="form-control rounded-3" id="floatingPassword" placeholder="Password" />
                                <label htmlFor="floatingPassword">Password</label>
                            </div>

                            <button className="w-100 mb-2 btn btn-lg rounded-3 btn-primary">
                                Login
                            </button>

                            <hr className="my-4" />

                            <h2 className="fs-5 fw-bold mb-3">Or use a third-party</h2>

                            <button type="button" className="w-100 py-2 mb-2 btn btn-outline-secondary rounded-3">
                                Login with Google
                            </button>

                            <button type="button" className="w-100 py-2 mb-2 btn btn-outline-primary rounded-3">
                                Login with Facebook
                            </button>

                            <button type="button" className="w-100 py-2 mb-2 btn btn-outline-secondary rounded-3">
                                Login with GitHub
                            </button>

                        </form>

                        <p className="mt-3">
                            Don't have an account yet?{" "}
                            <Link to="/register">Register</Link>
                        </p>

                    </div>
                </div>
            </div>
        </div>
        </>
    )
}